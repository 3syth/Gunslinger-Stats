 ...
 * if it could not add ability modifier to a bonus action attack.
 * 
 * Assumptions Made:
 * - Three attacks per combat (pistolero feat), no other abilities.
        ...
        for (int round = 0; round < rounds; round++) {
            for (int attack = 0; attack < 3; attack++) {
                ...
                if (d20 >= critThreshold) {
                    ...
                    if (attack != 2) { // not a bonus attack
                        damage += dexModifier;
                    }
                    totalDamage += damage;
                    if (level >= 14) {
